...
const Auth = {
    SignUp:async(req,res)=>{
        ...
        try {
            ...
            const token = jwt.sign({email:email},process.env.JWT_SCERET_KEY,{expiresIn:'1h'})//genrate token
            ...
            if(data){
              ...
              const user = new authSchema({//query to create new user
                name:name,
                email:email,
       ...
       try {
        ...
        if(data){
          ...
          if(passwordMatch===true){
            const token = jwt.sign({email:email},process.env.JWT_SCERET_KEY,{expiresIn:'1h'})
            res.status(200).json({
